...
def train_test_asset(RUN, filename, save_to="torch_model/model_final.pt"):
    seed = RUN["seed"]
    torch.manual_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    scaler = StandardScaler()
    sampler = get_sampler(run_conf["balance_algo"])
    data = compute_indicators_labels_lib.get_asset_dataset(RUN, filename)
    data["Date"] = pd.to_datetime(data["Date"])
    data = data[data["Date"] < RUN["back_test_start"]]
...
if __name__ == "__main__":
    train_test_asset(run_conf, "CHFUSD=X")
