...
from torch_training import train_test
...
def main():
    ...
    # run_conf["back_test_start"] = Timestamp("2023-10-01")  # in the future
    # run_conf["back_test_end"] = Timestamp("2024-01-10")  # in the past
    ...
    for bf in top_most:
        ...
        rep_rows, rep_fields = train_test(
            run_conf,
            "model_final_%d_%d.h5" % (run_conf["b_window"], run_conf["f_window"]),
        )
        df = pd.DataFrame(rep_rows, columns=rep_fields)
        df.to_excel(
            "reports/final_model_%d_%d.xlsx"
            % (run_conf["b_window"], run_conf["f_window"])
        )
    """
    rep_rows, rep_fields = train_test(
        run_conf,
    ...
    df = pd.DataFrame(rep_rows, columns=rep_fields)
    df.to_excel(
        "reports/final_model_%d_%d.xlsx" % (run_conf["b_window"], run_conf["f_window"])
    )
